...
if(!( function_exists( 'tommusrhodus_get_reading_time' ) )){
	function tommusrhodus_get_reading_time( $post_id ){
		...
		$min_read = esc_html__( ' min read', 'jumpstart' );
...
if(!( function_exists( 'get_tommusrhodus_breadcrumbs' ) )){ 
	function get_tommusrhodus_breadcrumbs( $class = 'breadcrumb' ){
		...
		if( 'career' == $post_type ){
			...
			$home .= '<li class="breadcrumb-item active"><a href="' . esc_url( home_url( "/". $slug ."/" ) ) . '">' . esc_html__( 'Careers', 'jumpstart' ) . '</a></li>';
		}

		if( 'testimonial' == $post_type ){
			$slug = get_option('tommusrhodus_testimonials_slug', 'testimonials');
			$home .= '<li class="breadcrumb-item active"><a href="' . esc_url( home_url( "/". $slug ."/" ) ) . '">' . esc_html__( 'Testimonials', 'jumpstart' ) . '</a></li>';
		}
		...
		if( tommusrhodus_is_blog_page() && is_single() ){
			$breadcrumb .= '<li class="breadcrumb-item"><a href="' . esc_url( get_post_type_archive_link( 'post' ) ) . '">'. get_option('stack_blog_title', 'Our Blog') .'</a></li><li class="breadcrumb-item active">'. get_the_title( $post->ID ) .'</li>';
		} elseif( tommusrhodus_is_blog_page() ){
			$breadcrumb .= '<li class="breadcrumb-item active">'. get_option('stack_blog_title','Our Blog') .'</li>';
...
if( !class_exists( 'TommusRhodus_Custom_Menu_Fields' ) ){
	...
	class TommusRhodus_Custom_Menu_Fields {
		...
		public static function init() {
			...
			self::$fields = array(
				'custom-icon' 		=> esc_html__( 'Select an Icon', 'jumpstart' ),
				'custom-image' 		=> esc_html__( 'Custom Image', 'jumpstart' ),
			);
