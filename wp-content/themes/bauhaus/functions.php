...
function bauhaus_theme_add_editor_styles() {
...
add_action( 'admin_menu', 'bauhaus_theme_add_editor_styles' );
...
class bauhaus_class {
	...
	function bauhaus_theme_support_setting() {
		...
		register_nav_menus(
			array(
				'bauhaus_header_top' => esc_html__( 'Header top menu', 'bauhaus' ),
 ...
 * @return bool
 */
function bauhaus_is_header_menu_enable() {
	$res = false;
	if ( get_theme_mod( 'bauhaus_performans_header_menu', false ) == true ) {
		$res = true;
	}
	if ( isset( $_GET['views'] ) && $_GET['views'] == 'top_menu' ) {
		$res = true;
	}
