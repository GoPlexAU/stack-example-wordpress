 ...
 */
function bauhaus_mailchimp_send()
{

    //get api kode
    preg_match("/(.*?)-(us..)/", sanitize_text_field(get_theme_mod('bauhaus_mailchimp_api_control')), $math);
    @$api_key = (isset($math[1])) ? $math[1] : "";
    if (@strlen($math[1]) < 10) {
        echo esc_attr( esc_html__(  'You have incorrect API key  ', 'bauhaus'));
        exit;
        die();

    }
    if (isset($math[2]) && strlen($math[2]) < 1) {
        echo esc_attr( esc_html__(  'You have incorrect dc ', 'bauhaus'));
        exit;
        die();
    }
    $list_id = sanitize_text_field(get_theme_mod('bauhaus_mailchimpid_list_control'));
    if (strlen($list_id) < 5) {
        echo esc_attr( esc_html__(  'You have incorrect id list ', 'bauhaus'));
        exit;
        die();
    }
...
add_action('wp_ajax_bauhaus_mailchimp_send', 'bauhaus_mailchimp_send');
add_action('wp_ajax_nopriv_bauhaus_mailchimp_send', 'bauhaus_mailchimp_send'); 
